    ...
    public partial class Form1 : Form
    {
        ...
        void Registrar()
        {
            try
            {
                if (txttitulo.Text !=""&& txtedicion.Text !=""&& cbgenero.Text!="")
                {
                    ...
                    Lib[indice].year.dia = Convert.ToInt32( txtdia.Text);
                    Lib[indice].year.mes = Convert.ToInt32( txtmes.Text);
                    Lib[indice].year.anyo = Convert.ToInt32( txtaño.Text);
                    indice++;
                }
            }
            catch (Exception e)
        ...
        void MostrarRegistros()
        {
            try
            {
                ...
                {
                    Lista = Lista + i + " - " + Lib[i].titulo + " , " + Lib[i].edicion + " , " + Lib[i].genero + " , " + Lib[i].autor + " , " + Lib[i].pais + " , " + Lib[i].year.dia + " , " + Lib[i].year.mes + " , " + Lib[i].year.anyo +"\n";
                }
                ...
                if (txtbuscar.Text !="")
                {
                    for (int i = 0; i < indice; i++)
                    {
                        if (Lib[i].titulo == txtbuscar.Text)
                        {
                            ...
                            txtdia.Text = Convert.ToString(Lib[i].year);
                            iModificar = i;
                        }
                    }
                }
        ...
        public void GuardarCambios()
        {
            try
            {
                ...
                if (txttitulo.Text != "" && txtedicion.Text != "" && cbgenero.Text != "")
                {
                    ...
                    Lib[iModificar].year.dia = Convert.ToInt32(txtdia.Text);
                    Lib[iModificar].year.mes = Convert.ToInt32(txtdia.Text);
                    Lib[iModificar].year.anyo = Convert.ToInt32(txtdia.Text);
